...
def run_pipeline(text: str, progress_callback=None) -> tuple:
    ...
    for i, chunk in enumerate(chunks):
        ...
        for sent in doc.sents:
            ...
            if len(sentence_text) < 5: continue
            ...
            # 1. Extract Entities & Noun Chunks
            ...
            for chunk_np in sent.noun_chunks:
                ...
                if len(np_words) >= 2 and zipf_frequency(np_key, 'en') < 3.5:
                    ...
                    for token in chunk_np: token_to_entity[token.i] = np_key
            ...
            if len(window_entities) > WINDOW_SIZE: window_entities.pop(0)
    ...
    for key, data in entity_map.items():
        ...
        if raw == "PERSON" or is_human_agent:
            # Re-check if it's a known natural entity mis-tagged as PERSON (e.g. 'Universe')
            if is_natural:
                best_label = "Location"
            else:
